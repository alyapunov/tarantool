...
test_mp_encode_tuple(void)
{
	...
	tuple_delete(tuple);
	char *w = expected;
	w = mp_encode_extl(w, MP_TUPLE, expected_len);
	w = mp_encode_uint(w, tuple->format_id);
	mp_encode_array(w, 0);
	is(memcmp(buf, expected, MIN(encoded_len, expected_len)), 0,
...
test_tuple_to_mpstream_as_ext(void)
{
	...
	is(memcmp(data, expected, MIN(data_len, expected_len)), 0,
...
test_mp_decode_tuple(void)
{
	...
	w = buf;
	w = mp_encode_extl(w, mp_extension_type_MAX, 1);
	r = buf;
	is(mp_decode_tuple(&r, &format_map), NULL,
	   "invalid MP_TUPLE is decoded correctly");
